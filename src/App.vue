...
import {normalizeArabic, waitForElement, truncate} from "@/helpers.js";
...
import { XMarkIcon, PauseIcon, PlayIcon, ArrowUpIcon, MagnifyingGlassIcon, ClipboardIcon, PlayCircleIcon, BookOpenIcon } from "@heroicons/vue/24/solid"
...
const surahSelector = ref(localStorage.getItem('surahSelector') ? parseInt(localStorage.getItem('surahSelector')) : 1);
...
async function copyToClipboard() {
  ...
  try {
    ...
    useToast().info(i18n.t('Ayah copied to clipboard!'));
  } catch (err) {
...
async function tafsirAyah(ayahNumber) {
  try {
    ...
    document.getElementById("tafsir-surah-name").textContent = normalizeArabic(data['data'][0].surah.name, false)
        + ' - ' + i18n.t('Ayah') + ' ' + data['data'][0].numberInSurah.toLocaleString(i18n.locale.value);
...
<template>
  ...
  <Transition>
    <div v-if="contentLoading" class="fixed z-50 w-screen h-screen bg-base-900/80 transition ease-in-out duration-100">
      <XMarkIcon class="size-6 m-4 float-end cursor-pointer" @click="contentLoading = false"/>
      <Spinner has-bg class="absolute top-1/2 left-1/2 scale-150"/>
    </div>
  </Transition>
  ...
  <AuthenticatedLayout>
          ...
          <ul class="py-2 text-sm text-base-700 dark:text-base-200">
            <li>
              <div @click="copyToClipboard"
                ...
                <ClipboardIcon class="rtl:mr-1 ltr:ml-1 size-6"/>
            ...
            <li>
              <div @click="showOptionsDropdown = !showOptionsDropdown; tafsirAyah(ayahClipboard[1])"
                   class="flex items-center p-1 text-center hover:bg-base-100 dark:hover:bg-base-600
                                        dark:hover:text-white cursor-pointer w-full">
                <BookOpenIcon class="rtl:mr-1 ltr:ml-1 size-6"/>
            ...
            <li>
              <div @click="showOptionsDropdown = !showOptionsDropdown; playAyah(ayahClipboard[1])"
                   class="flex items-center text-center hover:bg-base-100 dark:hover:bg-base-600
                                        dark:hover:text-white cursor-pointer w-full">
                <playIcon class="rtl:mr-2 ltr:ml-2 size-6"/>
        ...
        <ul v-if="ayahSearchResults.length > 0"
          ...
          <li v-for="ayah in ayahSearchResults"
            ...
            <p @click="jumpToAyah(ayah[3] - 1, ayah[4]) /*(surah number - ayah global number)*/">
              {{ ayah[0] }} - {{ ayah[1] }} - #{{ ayah[2].toLocaleString('ar-EG') }}
            </p>
          </li>
        </ul>
        <p v-else
    ...
    <Card>
      ...
      <div class="flex flex-col lg:flex-row h-full w-full justify-between items-center ">
        <!-- Reciter -->
        <div class="border-base-400 dark:border-base-900 w-full min-h-16 p-2">
          ...
          >
            ...
            <option v-for="reciter in availableReciters" :key="reciter.identifier"
              ...
              {{
                '[' + $t(reciter.language) + '] ' + ($i18n.locale === 'ar-EG' ? reciter.name : reciter.englishName)
              }}
            </option>
          </select>
        </div>
        ...
        <div class="border-base-400 dark:border-base-900 w-full min-h-16 p-2">
          ...
          <select id="tafsir-id" class="bg-base-50 border border-base-300 text-base-900 text-sm rounded-lg
                            focus:ring-primary-500 focus:border-primary-500 flex w-full dark:bg-base-700 dark:border-base-600
                            dark:placeholder-base-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 overflow-auto"
          ...
          >
            ...
            <option v-for="tafsir in availableTafsirs" :key="tafsir.identifier" :value="tafsir.identifier">
              {{
                '[' + $t(tafsir.language) + '] ' + ($i18n.locale === 'ar-EG' ? truncate(tafsir.name) : truncate(tafsir.englishName))
              }}
            </option>
          </select>
        </div>

        <!-- Tafsir / Translation Switch -->
        <div class="border-base-400 dark:border-base-900 w-full min-h-16 p-2">
          <div class="flex flex-row h-full">
            <div class="w-full h-full">
              <InputLabel for="taf-tar-switch" :value="$t('Show Tafsir / Translation under Ayahs')"/>
              <ul id="taf-tar-switch" class="items-center w-full text-sm font-medium text-base-900 bg-white border border-base-200 rounded-lg
                                        flex dark:bg-base-700 dark:border-base-600 dark:text-white h-full">
                <li class="w-full rtl:border-l ltr:border-r dark:border-base-600">
                  <div class="flex items-center pl-1">
                    ...
                    <label for="horizontal-list-radio-on"
                           class="w-full py-2 ml-2 text-sm font-medium text-base-900 dark:text-base-300 mx-2">{{ $t('On') }}</label>
                  </div>
                </li>
                <li class="w-full">
                  <div class="flex items-center pl-1">
                    ...
                    <label for="horizontal-list-radio-off"
                           class="w-full py-2 ml-2 text-sm font-medium text-base-900 dark:text-base-300  mx-2">{{ $t('Off') }}</label>
                  </div>
                </li>
              </ul>
            </div>
    ...
    <Card class="font-kitab !mb-24">
      ...
      <div class="text-center">
        <h1 class="font-bold text-5xl p-3">
        ...
        <div @click="playSurah()" class="inline-block p-1 hover:bg-base-700 rounded-lg cursor-pointer line-h transition ease-in-out duration-150">
          <PlayCircleIcon class="size-4 inline-block mx-1"/>
          <span class="">{{ $t('Play Surah') }}</span>
        </div>
    ...
    <div class="flex justify-center">
      ...
      <div class="fixed bottom-8 w-3/4 sm:w-1/2 h-16 bg-amber-400 rounded-full text-center shadow-md">
        <div class="flex items-center justify-between mx-3 h-full" dir="ltr">
          ...
          <div class="flex flex-1 items-center gap-2">
            ...
            <span v-show="!isLoading" class="hidden sm:block" id="ayah-duration">0:00 / 0:00</span>
          </div>

          <!--Play Full Surah-->
          <div class="hidden lg:flex lg:flex-1 justify-center">
            <button id="play-surah-btn" @click="playSurah()"
                    class="hidden lg:flex  text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none
                            focus:ring-primary-300 font-medium rounded-full text-sm px-3 py-2.5 text-center
                            items-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
            >
              <PlayCircleIcon class="size-6" v-if="$i18n.locale === 'en-US'"/>
              <span class="mx-2">{{ $t("Play Full Surah") }}</span>
              <PlayCircleIcon class="size-6" v-if="$i18n.locale === 'ar-EG'"/>
            </button>
          </div>
    ...
    <TransitionGroup>
      <div class="fixed top-8 lg:bottom-8 ltr:ml-6 rtl:mr-6 bg-primary-500 w-fit h-fit p-2 rounded-full shadow-md cursor-pointer
                    hover:bg-primary-400 transition ease-in-out duration-200"
           id="scroll-to-top-btn"
           @click="scrollToTop"
           v-if="scrollTracker > 350"
      >
